...
class DynInvoke(Invoke):
    ...
    def gen_code(self, parent):
        ...
        # Whether or not we are to create a de-referencing routine.
        # If we are, then we create PSy1 in which all derived types
        # etc. are 'unpacked'. This then calls PSy2 which deals only
        # with intrinsic Fortran types.  If we're not then PSy1
        # contains the loop(s) over cells and calls the kernel routine(s)
        # directly.
        make_deref_routine = self.schedule.deref_routine
        ...
        if len(operator_declarations) > 0:
            invoke_sub_psy1.add(TypeDeclGen(invoke_sub_psy1,
                                            datatype="operator_type",
                                            entity_decls=operator_declarations,
                                            intent="inout"))
        # qr
        if len(self._psy_unique_qr_vars) > 0:
            invoke_sub_psy1.add(
                TypeDeclGen(invoke_sub_psy1,
                            datatype="quadrature_type",
        ...
        for fld in fields:
            ...
            if psy2_args:
                ...
                # list and declare them within PSy2. We declare them
                # separately as we are specifying the extent of the array
        ...
        for function_space in self.unique_fss():
            ...
            invoke_sub_psy1.add(
                AssignGen(invoke_sub_psy1, lhs=ndf_name,
                          rhs=name_on_space+"%" +
                          arg_on_space.ref_name(function_space)+"%get_ndf()"))
            ...
            if function_space in field_on_space.keys():
                ...
                if make_deref_routine:
                    # We must also pass it to PSy2 if that is being
                    # created
                    psy2_caller_args.append(undf_name)
                    psy2_dummy_args.append(undf_name)
                invoke_sub_psy1.add(
                    AssignGen(invoke_sub_psy1,
                              lhs=undf_name,
                              rhs=name_on_space + "%" +
                              arg_on_space.ref_name(function_space) +
                              "%get_undf()"))
                ...
                invoke_sub_psy1.add(
                    AssignGen(invoke_sub_psy1, lhs=dmap_name,
                              pointer=True,
                              rhs=name_on_space + "%" +
                              arg_on_space.ref_name(function_space) +
                              "%get_dofmap()"))
            ...
            if self.basis_required(function_space):
                ...
                rhs = \
                    name_on_space + "%" + \
                    arg_on_space.ref_name(function_space) + "%get_dim_space()"
                invoke_sub_psy1.add(
                ...
                if make_deref_routine:
                    psy2_operator_declarations.append(op_name + "(" +
                                                      alloc_args + ")")
                ...
                if make_deref_routine:
                    invoke_sub_psy2.add(
                        DeclGen(invoke_sub_psy2,
                                ...
                                intent="in",
                                dimension=self.
                                ndf_name(function_space) + ",ncells",
                                entity_decls=[fs_descriptor.
                                              orientation_name]))
        ...
        if bc_kern:
            ...
            if invoke_sub_psy2 and bc_kern.name == "matrix_vector_mm_code":
        ...
        if make_deref_routine:
            ...
            self.schedule.gen_code(invoke_sub_psy2)

        else:
            # We call the kernel directly from this routine
            invoke_sub_psy1.add(CommentGen(invoke_sub_psy1, ""))
            invoke_sub_psy1.add(CommentGen(invoke_sub_psy1,
                                           " Call our kernels"))
            invoke_sub_psy1.add(CommentGen(invoke_sub_psy1, ""))
            # add content from the schedule
            self.schedule.gen_code(invoke_sub_psy1)

        # Any clean-up after the kernel call(s) goes here
        if self.qr_required:
            ...
            invoke_sub_psy1.add(DeallocateGen(invoke_sub_psy1,
                                              func_space_var_names))
        invoke_sub_psy1.add(CommentGen(invoke_sub_psy1, ""))

        # Finally, add the PSy routine(s) to my parent
        parent.add(invoke_sub_psy1)
        if make_deref_routine:
            parent.add(invoke_sub_psy2)
...
class DynKern(Kern):
    ...
    @property
    def _deref_routine(self):
        ''' Dynamically look-up whether or not we are generating a
        de-referencing routine for the Schedule that calls this kernel '''
        # Store a reference to our Schedule. This enables us to look-up
        # whether we're generating a de-referencing routine at
        # code-generation time.
        if not self._schedule:
            self._schedule = self.ancestor(DynSchedule)
        return self._schedule.deref_routine
    ...
    def _create_arg_list(self, parent, my_type="call"):
        ...
        if my_type == "subroutine":
            ...
            parent.add(UseGen(parent, name="constants_mod", only="True",
                              funcnames=["r_def"]))
        # We'll only need this 'name separator' if we're generating a
        # call rather than the routine itself. In the former case
        # we are typically (always?) called from the stub generator in
        # which case we don't have to worry about how the kernel is
        # being called (i.e. whether or not we have a de-referencing routine)
        name_sep = ""
        if my_type != "subroutine" and self._deref_routine:
            # If we're using a dereferencing routine then we cannot be
            ...
            if self.name.lower() == "enforce_bc_code" and \
               unique_fs.lower() == "any_space_1":
                ...
                if my_type == "call" and not self._deref_routine:
                    ...
                    proxy_name = \
                        self._arguments.\
                        get_arg_on_space("any_space_1").proxy_name
    ...
    def gen_code(self, parent):
        ...
        # it. Happily however, declarations are automatically
        ...
        if not self.module_inline:
            # Use-statements too are bubbled-up, just like declarations
        ...
        if self.name == "matrix_vector_mm_code":
            # In matrix_vector_mm_code, all fields are on the same
            # (unknown) space. Therefore we can use any field to
            # dereference. We choose the first one.
            ...
            map_name = \
                self.fs_descriptors.map_name(kern_func_space_name) + \
            ...
            enforce_name = enforce_bc_arg.proxy_name
            if not self._deref_routine:
                enforce_name += "%data"
...
class DynKernelArgument(Argument):
    ...
    @property
    def descriptor(self):
        ''' Returns the kernel-argument descriptor which contains the
        kernel meta-data describing this argument '''
        return self._arg
