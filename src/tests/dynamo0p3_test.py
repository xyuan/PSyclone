...
def test_fs_repr():
    '''Test that the __repr__ and __str__ methods of DynFuncDescriptor03
    ...
    fsdesc = metadata._func_descriptors[0]
    assert repr(fsdesc) == "DynFuncDescriptor03(func_type(w1, gh_basis))"
    ...
    fsdesc_str = str(fsdesc)
    assert expected in fsdesc_str
...
def test_field():
    ...
    output = (
        ...
        "      map_w1 => f1_proxy%vspace%get_dofmap()\n"
        "      !\n"
        "      ! Initialise sizes and allocate any basis arrays for w2\n"
        "      !\n"
        "      ndf_w2 = f2_proxy%vspace%get_ndf()\n"
        "      undf_w2 = f2_proxy%vspace%get_undf()\n"
        "      map_w2 => f2_proxy%vspace%get_dofmap()\n"
        "      !\n"
        "      ! Initialise sizes and allocate any basis arrays for w3\n"
        "      !\n"
        "      ndf_w3 = m2_proxy%vspace%get_ndf()\n"
        "      undf_w3 = m2_proxy%vspace%get_undf()\n"
        "      map_w3 => m2_proxy%vspace%get_dofmap()\n"
        "      !\n"
        "      ! Call de-referenced PSy layer\n"
        "      !\n"
        "      CALL invoke_0_testkern_type_arrays(f1_proxy%data, "
        "f2_proxy%data, m1_proxy%data, m2_proxy%data, ncells, nlayers, "
        "ndf_w1, undf_w1, map_w1, ndf_w2, undf_w2, map_w2, ndf_w3, undf_w3, "
        "map_w3)\n"
        "      !\n"
        ...
        "      INTEGER, intent(in), dimension(ndf_w1,0:ncells) :: map_w1\n"
        "      INTEGER, intent(in), dimension(ndf_w2,0:ncells) :: map_w2\n"
        "      INTEGER, intent(in), dimension(ndf_w3,0:ncells) :: map_w3\n"
        "      INTEGER, intent(in) :: ndf_w1, undf_w1, ndf_w2, undf_w2, "
        "ndf_w3, undf_w3\n"
        "      INTEGER cell\n"
        "      DO cell=1,ncells\n"
        "        CALL testkern_code(nlayers, f1_proxy, f2_proxy, "
        "m1_proxy, m2_proxy, ndf_w1, undf_w1, map_w1(:,cell), ndf_w2, "
        "undf_w2, map_w2(:,cell), ndf_w3, undf_w3, map_w3(:,cell))\n"
        "      END DO \n"
        "    END SUBROUTINE invoke_0_testkern_type_arrays\n"
...
def test_field_no_deref():
    ''' Tests that a call with a set of fields and no basis
    functions produces correct code without a de-referencing routine. '''
    _, invoke_info = parse(os.path.join(BASE_PATH, "1_single_invoke.f90"),
                           api="dynamo0.3")
    psy = PSyFactory("dynamo0.3").create(invoke_info)
    invoke = psy.invokes.invoke_list[0]
...
def test_field_fs():
    ...
    output = (
        ...
        "      INTEGER, intent(in), dimension(ndf_w2,0:ncells) :: map_w2\n"
        "      INTEGER, intent(in), dimension(ndf_w3,0:ncells) :: map_w3\n"
        "      INTEGER, intent(in), dimension(ndf_wtheta,0:ncells) :: "
        "map_wtheta\n"
        "      INTEGER, intent(in), dimension(ndf_w2h,0:ncells) :: map_w2h\n"
        "      INTEGER, intent(in), dimension(ndf_w2v,0:ncells) :: map_w2v\n"
        "      INTEGER, intent(in) :: ndf_w1, undf_w1, ndf_w2, undf_w2, "
        ...
        "m2_proxy, f3_proxy, f4_proxy, m3_proxy, "
        ...
        "      END DO \n"
        "    END SUBROUTINE invoke_0_testkern_fs_type_arrays\n"
        "  END MODULE psy_single_invoke_fs")
    print str(generated_code)
    assert str(generated_code).find(output) != -1


def test_field_fs_no_deref():
    ''' Tests that a call with a set of fields making use of all
    function spaces and no basis functions produces correct code when we
    ...
    output = (
        ...
        "m3_proxy%data, ndf_w1, undf_w1, map_w1(:,cell), "
...
def test_field_qr():
    ...
    output = (
        ...
        "      ! Call de-referenced PSy layer\n"
        "      !\n"
        "      CALL invoke_0_testkern_qr_type_arrays(f1_proxy%data, "
        "f2_proxy%data, m1_proxy%data, m2_proxy%data, ncells, nlayers, "
        "nqp_h, nqp_v, wh, wv, ndf_w1, undf_w1, map_w1, dim_w1, basis_w1, "
        "ndf_w2, undf_w2, map_w2, diff_dim_w2, diff_basis_w2, ndf_w3, "
        "undf_w3, map_w3, dim_w3, basis_w3, diff_dim_w3, diff_basis_w3)\n"
        "      !\n"
        ...
        "    SUBROUTINE invoke_0_testkern_qr_type_arrays(f1_proxy, f2_proxy, "
        "m1_proxy, m2_proxy, ncells, nlayers, nqp_h, nqp_v, wh, wv, ndf_w1, "
        "undf_w1, map_w1, dim_w1, basis_w1, ndf_w2, undf_w2, map_w2, "
        "diff_dim_w2, diff_basis_w2, ndf_w3, undf_w3, map_w3, dim_w3, "
        "basis_w3, diff_dim_w3, diff_basis_w3)\n"
        ...
        "      INTEGER, intent(in) :: ndf_w1, undf_w1, ndf_w2, undf_w2, "
        "ndf_w3, undf_w3\n"
        "      INTEGER, intent(in) :: dim_w1, diff_dim_w2, dim_w3, "
        "diff_dim_w3\n"
        "      REAL(KIND=r_def), intent(in) :: basis_w1(dim_w1, ndf_w1, "
        "nqp_h, nqp_v), diff_basis_w2(diff_dim_w2, ndf_w2, nqp_h, nqp_v), "
        "basis_w3(dim_w3, ndf_w3, nqp_h, nqp_v), diff_basis_w3(diff_dim_w3, "
        "ndf_w3, nqp_h, nqp_v)\n"
        "      INTEGER cell\n"
        "      REAL(KIND=r_def), dimension(:) :: wh, wv\n"
        "      DO cell=1,ncells\n"
        "        CALL testkern_qr_code(nlayers, f1_proxy, f2_proxy, m1_proxy,"
        " m2_proxy, ndf_w1, undf_w1, map_w1(:,cell), basis_w1, ndf_w2, "
        "undf_w2, map_w2(:,cell), diff_basis_w2, ndf_w3, undf_w3, "
        "map_w3(:,cell), basis_w3, diff_basis_w3, nqp_h, nqp_v, wh, wv)\n"
        "      END DO \n"
        "    END SUBROUTINE invoke_0_testkern_qr_type_arrays\n"
        "  END MODULE psy_single_invoke")
...
def test_field_qr_no_deref():
    ''' Tests that a call, with a set of fields requiring
    quadrature, produces correct code without a de-referencing routine. '''
    _, invoke_info = parse(os.path.join(BASE_PATH,
                                        "1.1_single_invoke_qr.f90"),
                           api="dynamo0.3")
    psy = PSyFactory("dynamo0.3").create(invoke_info)
...
def test_orientation():
    ...
    generated_code = str(psy.gen)
    print generated_code
    assert generated_code.find("INTEGER, pointer :: orientation_w2(:,:)"
                               " => null()") != -1
    assert generated_code.find("orientation_w2 => f2_proxy%vspace%"
                               "get_orientation()") != -1
...
def test_operator_no_deref():
    ...
    assert generated_code.find(
        ...
        "w0_proxy%local_stencil, chi_proxy(1)%data, chi_proxy(2)%data, "
        "chi_proxy(3)%data, ndf_w0, undf_w0, map_w0(:,cell), basis_w0, "
...
def test_operator_different_spaces():
    ...
    output = (
        ...
        "      INTEGER nqp_h, nqp_v\n"
        "      INTEGER ncells, nlayers\n"
        "      REAL(KIND=r_def), pointer :: mapping_proxy_local_stencil(:,:,:)"
        " => null()\n"
        ...
        "      mapping_proxy_local_stencil => mapping_proxy%local_stencil\n"
        "      !\n"
        "      ! Initialise number of cells & layers\n"
        "      !\n"
        "      nlayers = mapping_proxy%fs_from%get_nlayers()\n"
        "      ncells = mapping_proxy%fs_from%get_ncell()\n"
        "      !\n"
        "      ! Initialise qr values\n"
        "      !\n"
        "      wv => qr%get_wqp_v()\n"
        ...
        "      ALLOCATE (diff_basis_w2(diff_dim_w2, ndf_w2, nqp_h, nqp_v))\n"
        "      orientation_w2 => mapping_proxy%fs_from%get_orientation()\n"
        "      !\n"
        ...
        "      map_w0 => chi_proxy(1)%vspace%get_dofmap()\n"
        ...
        "basis_w3, ndf_w2, diff_basis_w2, orientation_w2(:,cell), ndf_w0, "
...
def test_operator_nofield_no_deref():
    ...
    assert code.find(
        ...
        ", chi_proxy(3)%data, ndf_w2, basis_w2, "
...
def test_operator_orientation_no_deref():
    ...
    assert gen_str.find(
        ...
        " chi_proxy(3)%data, ndf_w1, basis_w1, orientation_w1(:,cell), "
        "ndf_w0, undf_w0, map_w0(:,cell), diff_basis_w0, "
        "nqp_h, nqp_v, wh, wv)") != -1
...
def test_any_space_1():
    ...
    assert generated_code.find(
        ...
        ", c_proxy_1, c_proxy_2, c_proxy_3, "
        "ndf_any_space_1, undf_any_space_1, map_any_space_1(:,cell), "
        "basis_any_space_1, ndf_any_space_2, undf_any_space_2, "
        "map_any_space_2(:,cell), basis_any_space_2, "
        "ndf_w0, undf_w0, map_w0(:,cell), diff_basis_w0, "
        "nqp_h, nqp_v, wh, wv)") != -1
    assert generated_code.find(
...
def test_any_space_1_no_deref():
    ...
    assert generated_code.find(
        ...
        "data, c_proxy(1)%data, c_proxy(2)%data, c_proxy(3)%data, "
        "ndf_any_space_1, undf_any_space_1, map_any_space_1(:,cell), "
        "basis_any_space_1, ndf_any_space_2, undf_any_space_2, "
        "map_any_space_2(:,cell), basis_any_space_2, "
        "ndf_w0, undf_w0, map_w0(:,cell), diff_basis_w0, nqp_h, nq"
        "p_v, wh, wv)") != -1
    assert generated_code.find(
        "DEALLOCATE (basis_any_space_1, basis_any_space_2, diff_basis_w"
        "0)") != -1
...
def test_operator_any_space_different_space_2():
    ...
    assert str(generated_code).find(
        "map_any_space_5 => a_proxy%vspace%get_dofmap()") != -1
    assert str(generated_code).find(
        "map_any_space_4 => d_proxy%fs_from%get_dofmap()") != -1
