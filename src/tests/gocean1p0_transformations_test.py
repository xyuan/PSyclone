...
def test_const_loop_bounds_invalid_offset():
    ...
    psy, invoke = get_invoke(
        os.path.join("gocean1p0", "test26_const_bounds_invalid_offset.f90"),
        API, 0)
    cbtrans = GOConstLoopBoundsTrans()
    schedule = invoke.schedule
...
def test_loop_fuse_different_iterates_over():
    ...
    _, invoke = get_invoke(
        os.path.join("gocean1p0", "test11_different_iterates_over_"
                     "one_invoke.f90"), API, 0)
    schedule = invoke.schedule
...
def test_loop_fuse_unexpected_error():
    ...
    _, invoke = get_invoke(
        os.path.join("gocean1p0", "test14_module_inline_same_kernel.f90"),
                     API, 0)
    schedule = invoke.schedule
...
def test_omp_region_nodes_not_children_of_same_schedule():
    ...
    to put a region around nodes that are not children of
    the same schedule '''
    _, invoke1 = get_invoke(
        os.path.join("gocean1p0",
                     "test12_two_invokes_two_kernels.f90"), API, 0)
    schedule1 = invoke1.schedule
    _, invoke2 = get_invoke(
        os.path.join("gocean1p0",
                     "test12_two_invokes_two_kernels.f90"), API, 1)
